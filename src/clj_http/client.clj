...
(defn wrap-output-coercion
  "Coerces the body in the response output to byte array. If the output coercion is
   not specified, the default is to convery the body to a String using the UTF-8
   character set.

   Trigger parameter in the request map is ':as' and valid values are: ':byte-array'"
  [client]
  (fn [{:keys [as] :as req}]
